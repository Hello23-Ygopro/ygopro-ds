--BT5-026 Righteous Heart Son Gohan
local scard,sid=aux.GetID()
function scard.initial_effect(c)
	aux.AddCharacter(c,CHARACTER_SON_GOHAN_ADOLESCENCE,CHARACTER_SHENRON)
...
function scard.actfilter(c,cost)
	if not (c:IsColor(COLOR_BLUE+COLOR_BLACK) and c:IsSpecialTrait(TRAIT_DESIRE) and c:IsEnergyBelow(cost)) then return false end
...
function scard.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif opt==3 then
		...
		local g2=Duel.SelectMatchingCard(tp,aux.EnergyAreaFilter(scard.tapfilter),tp,LOCATION_ENERGY,0,0,3,nil,e)
		...
		--critical
		aux.AddTempSkillCritical(c,c,6)
		--flip over
