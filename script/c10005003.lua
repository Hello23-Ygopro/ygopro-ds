--BT5-002 Oolong, Always Wanting More
local scard,sid=aux.GetID()
function scard.initial_effect(c)
	aux.AddCharacter(c,CHARACTER_OOLONG)
	aux.AddSpecialTrait(c,TRAIT_EARTHLING,TRAIT_SHENRON)
...
function scard.actfilter(c,cost)
	if not (c:IsColor(COLOR_RED+COLOR_BLACK) and c:IsSpecialTrait(TRAIT_DESIRE) and c:IsEnergyBelow(cost)) then return false end
...
function scard.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif opt==3 then
		...
		local g2=Duel.SelectMatchingCard(tp,aux.BattleAreaFilter(Card.IsCanBeEffectTarget),tp,LOCATION_BATTLE,0,1,1,nil,e)
		if g2:GetCount()>0 then
			Duel.SetTargetCard(g2)
			--gain power
			aux.AddTempSkillUpdatePower(c,g2:GetFirst(),5,15000)
		end
		--flip over
